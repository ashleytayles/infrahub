...
class SchemaManager(NodeManager):
    ...
    async def update_schema_branch(
        ...
        if update_db:
            if diff:
                schema_diff = await self.update_schema_to_db(schema=schema, db=db, branch=branch, diff=diff, at=at)
            else:
                await self.load_schema_to_db(schema=schema, db=db, branch=branch, limit=limit, at=at)
    ...
    async def update_schema_to_db(
        ...
        for item_kind in diff.added.keys():
            ...
            node = await self.load_node_to_db(node=item, branch=branch, db=db, at=at)
        ...
        for item_kind in diff.removed.keys():
            item = schema.get(name=item_kind, duplicate=False)
            node = await self.delete_node_in_db(node=item, branch=branch, db=db, at=at)
            ...
            for item in node.attributes:
                if item.inherited is False:
                    new_attr = await self.create_attribute_in_db(
                        schema=attribute_schema, item=item, parent=obj, branch=branch, db=db, at=at
                    )
                else:
            ...
            for item in node.relationships:
                if item.inherited is False:
                    new_rel = await self.create_relationship_in_db(
                        schema=relationship_schema, item=item, parent=obj, branch=branch, db=db, at=at
                    )
                else:
    ...
    async def update_node_in_db(
        ...
        for item in node.local_attributes:
            if item.id and item.id in items:
                await self.update_attribute_in_db(item=item, attr=items[item.id], db=db, at=at)
            elif not item.id:
                new_attr = await self.create_attribute_in_db(
                    schema=attribute_schema, item=item, branch=branch, db=db, parent=obj, at=at
                )
                new_node.attributes.append(new_attr)

        for item in node.local_relationships:
            if item.id and item.id in items:
                await self.update_relationship_in_db(item=item, rel=items[item.id], db=db, at=at)
            elif not item.id:
                new_rel = await self.create_relationship_in_db(
                    schema=relationship_schema, item=item, branch=branch, db=db, parent=obj, at=at
                )
                new_node.relationships.append(new_rel)
        ...
        if diff_attributes:
            for item in node.local_attributes:
                ...
                if item.name in diff_attributes.added or (item.name in diff_attributes.changed and item.id is None):
                    ...
                    await items[item.id].delete(db=db, at=at)
                elif (
        ...
        if field_names_to_remove:
            for field_schema in items.values():
                if field_schema.name.value in field_names_to_remove:
                    await field_schema.delete(db=db, at=at)
    ...
    async def delete_node_in_db(
        ...
        for attr_schema_node in (await obj.attributes.get_peers(db=db)).values():
            await attr_schema_node.delete(db=db, at=at)
        for rel_schema_node in (await obj.relationships.get_peers(db=db)).values():
            await rel_schema_node.delete(db=db, at=at)
    ...
    async def update_relationship_in_db(
        item: RelationshipSchema, rel: Node, db: InfrahubDatabase, at: Timestamp
    ) -> None:
        item_dict = item.model_dump(exclude={"id", "state", "filters"})
        for key, value in item_dict.items():
            getattr(rel, key).value = value
        await rel.save(db=db, at=at)
